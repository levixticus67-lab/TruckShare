...
function FreightPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {open && <Modal title="Post a load" eyebrow="Shipper portal" onClose={() => setOpen(false)}><form onSubmit={submit} className="space-y-4"><div className="grid gap-4 sm:grid-cols-2"><LocationPicker label="Pickup area" value={form.pickupLocation} countryCode={form.pickupCountry} onChange={setPickup} /><CountrySelect label="Pickup country" value={form.pickupCountry} onChange={setPickupCountry} /><LocationPicker label="Drop-off area" value={form.dropoffLocation} countryCode={form.dropoffCountry} onChange={setDropoff} /><CountrySelect label="Drop-off country" value={form.dropoffCountry} onChange={setDropoffCountry} /><Field label="Cargo description" value={form.description} onChange={(value) => update("description", value)} placeholder="Bagged maize, cartons..." /><Field label="Cargo type" value={form.cargoType} onChange={(value) => update("cargoType", value)} /><Field label="Weight (tons)" type="number" value={form.weightTons} onChange={(value) => update("weightTons", value)} /><Field label="Volume (m³)" type="number" value={form.volumeM3} onChange={(value) => update("volumeM3", value)} /><Field label="Pickup date" type="date" value={form.pickupDate} onChange={(value) => update("pickupDate", value)} /><Field label="Budget" type="number" value={form.price} onChange={(value) => update("price", value)} /><label><span className={labelClass}>Budget currency</span><select className={input} value={form.currency} onChange={(event) => update("currency", event.target.value)}>{regionalReference.countries.map((country) => <option key={country.currency} value={country.currency}>{country.currency} · {country.name}</option>)}</select></label></div><p className="text-[11px] text-muted-foreground">Pin the actual pickup and delivery area so matching and routing do not assume the city centre.</p><button className={`${button} w-full`}><Plus size={14} /> Publish load</button></form></Modal>}
    </div>
  );
}
