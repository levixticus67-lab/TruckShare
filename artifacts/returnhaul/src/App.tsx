...
function MatchesPage() {
  return <div className="mx-auto max-w-4xl space-y-6"><Header eyebrow="Broker-managed matching" title="Your request status" detail="TruckShare’s operations team reviews every load and trip, selects the match, and sends the proposed arrangement for confirmation." action={<Link href="/messages" className={button}><MessageSquare size={14} /> Contact operations</Link>} /><Card className="border-primary/15 bg-[#e5eee9] text-primary"><div className="flex items-start gap-3"><ShieldCheck className="mt-0.5 text-[#28765a]" size={22} /><div><h3 className="font-display text-xl font-semibold">No self-service matching</h3><p className="mt-2 text-sm leading-6 text-primary/70">You will receive an update when the broker has reviewed your request, found a suitable counterpart, and is ready to send an offer. This keeps pricing, verification, and operational details under one accountable desk.</p></div></div></Card><div className="grid gap-4 md:grid-cols-3"><Card><p className={labelClass}>Step 1</p><h3 className="mt-2 font-display text-lg font-semibold">Submit</h3><p className="mt-1 text-xs leading-5 text-muted-foreground">Send your load or available capacity with route and timing.</p></Card><Card><p className={labelClass}>Step 2</p><h3 className="mt-2 font-display text-lg font-semibold">Broker review</h3><p className="mt-1 text-xs leading-5 text-muted-foreground">The operations team qualifies the request and finds the best fit.</p></Card><Card><p className={labelClass}>Step 3</p><h3 className="mt-2 font-display text-lg font-semibold">Confirm</h3><p className="mt-1 text-xs leading-5 text-muted-foreground">Review the proposed arrangement before it becomes a booking.</p></Card></div><div className="flex flex-wrap gap-2"><Link href="/bookings" className={secondaryButton}>View assigned bookings <ArrowRight size={14} /></Link><Link href="/account" className={secondaryButton}>Open my account <UserRound size={14} /></Link></div></div>;
}
