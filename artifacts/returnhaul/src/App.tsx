...
function Stat({ label, value, note, icon: Icon, accent = false }: { label: string; value: string | number; note: string; icon: typeof Activity; accent?: boolean }) { return <Card className={accent ? "border-accent/40 bg-[#fff5e3]" : ""}><div className="flex items-start justify-between"><div><p className="font-mono-ui text-[9px] uppercase tracking-[.14em] text-muted-foreground">{label}</p><p className="mt-3 font-display text-3xl font-semibold tracking-[-.05em]">{value}</p><p className="mt-1 text-[11px] text-muted-foreground">{note}</p></div><span className="flex h-9 w-9 items-center justify-center rounded-lg bg-muted text-primary"><Icon size={17} /></span></div></Card>; }
