...
type AdminOperations = {
  ...
  metrics: { totalOpen: number; newSubmissions: number; needsAttention: number; matching: number; activeOperations: number; pendingVerifications: number; activeBookings: number; grossVolume: number; commissionValue: number; unpaidBookings: number; borderIssues: number };
  actor?: AuthUser | null;
};
...
const button = "inline-flex items-center justify-center gap-2 rounded-full bg-primary px-4 py-2.5 text-xs font-bold text-primary-foreground shadow-[0_10px_24px_hsl(var(--primary)/.24)] transition hover:-translate-y-0.5 hover:bg-primary/90 disabled:cursor-not-allowed disabled:opacity-50";
const secondaryButton = "inline-flex items-center justify-center gap-2 rounded-full border border-border bg-card/70 px-4 py-2.5 text-xs font-bold shadow-sm transition hover:-translate-y-0.5 hover:bg-card disabled:opacity-50";
const input = "h-11 w-full rounded-xl border border-input bg-background/60 px-3.5 text-sm outline-none focus:border-accent focus:ring-2 focus:ring-accent/20";
...
function Shell({ children }: { children: ReactNode }) {
  ...
  return <RoleContext.Provider value={role}><div className="app-shell noise min-h-[100dvh] bg-background">
    <aside className={`app-sidebar fixed inset-y-0 left-0 z-40 flex w-[258px] flex-col bg-sidebar px-4 py-5 text-sidebar-foreground shadow-2xl transition-transform lg:translate-x-0 ${mobileOpen ? "translate-x-0" : "-translate-x-full"}`}>
...
function Header({ eyebrow, title, detail, action }: { eyebrow: string; title: string; detail?: string; action?: ReactNode }) {
  ...
  return <div className="page-header flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div className="min-w-0"><p className="font-mono-ui text-[10px] uppercase tracking-[.16em] text-accent-foreground/65">{simpleEyebrow[eyebrow] || eyebrow}</p><h2 className="mt-1 break-words font-display text-3xl font-semibold tracking-[-.045em]">{simpleTitle[title] || title}</h2>{simpleDetail && <p className="mobile-page-detail mt-2 max-w-2xl text-sm text-muted-foreground">{simpleDetail}</p>}</div>{action && <div className="page-header-action shrink-0">{action}</div>}</div>;
}
...
function ContextImage({ src, alt, eyebrow, title }: { src: string; alt: string; eyebrow: string; title: string }) {
  return <div className="context-image relative min-h-[170px] overflow-hidden rounded-xl border border-border shadow-sm sm:min-h-[205px]">
    <img src={src} alt={alt} className="absolute inset-0 h-full w-full object-cover" loading="lazy" />
    <div className="absolute inset-0 bg-gradient-to-t from-[#0d1b26]/80 via-[#0d1b26]/15 to-transparent" />
    <div className="context-image-copy absolute inset-x-0 bottom-0 p-5 text-white sm:p-6">
      <p className="font-mono-ui text-[9px] uppercase tracking-[.16em] text-white/70">{eyebrow}</p>
      <p className="mt-1 max-w-md font-display text-xl font-semibold tracking-[-.03em]">{title}</p>
    </div>
  </div>;
}
  ...
  return <>
    <Card className="logistics-card overflow-hidden p-0 transition-shadow hover:-translate-y-0.5 hover:shadow-xl">
      <div className="p-5 sm:p-6">
        ...
        <div className="cargo-card-visual mt-4"><img src="/branding/story/cargo-loading.jpg" alt="" /><div className="cargo-card-visual-copy"><p className={labelClass}>Broker-managed load</p><p className="font-display text-lg font-semibold">{load.cargoType || "General cargo"}</p><p className="text-[11px] text-muted-foreground">{load.weightTons} tons · {load.volumeM3 || "—"} m³</p></div></div>
        <div className="mt-5 grid grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-center gap-2">
          <div className="min-w-0">
            <p className="font-mono-ui text-[9px] font-bold uppercase tracking-[.16em] text-accent-foreground">From</p>
      ...
      <div className="flex items-center gap-2 p-4 sm:p-5">
        <button type="button" onClick={() => setRouteOpen(true)} className={`${secondaryButton} flex-1`}><MapPin size={14} /> View route</button>
         <Link href="/matches" className={`${button} flex-1`}>View broker status <Search size={14} /></Link>
      </div>
    </Card>
...
function TripsPage() {
  ...
  return (
    <div className="space-y-6">
      <Header eyebrow="Carrier submission desk" title="My trip submissions" detail="Submit your available capacity and our broker team will review the route, find a load, and coordinate the assignment." action={role === "Carrier" ? <button onClick={() => setOpen(true)} className={button}><Plus size={14} /> Submit trip capacity</button> : undefined} />
      <ContextImage src="/branding/story/carrier-road.jpg" alt="Truck travelling along a winding road" eyebrow="Broker-managed capacity" title="You provide the route. The broker handles the match." />
      {role === "Shipper" ? <Card><div className="flex items-start gap-3"><ShieldCheck className="mt-0.5 text-accent-foreground" size={20} /><div><h3 className="font-display text-xl font-semibold">Truck assignments go through the broker desk</h3><p className="mt-1 text-sm leading-6 text-muted-foreground">Submit your load from My loads. The operations team will select and propose the carrier instead of exposing an open truck board.</p><Link href="/freight" className={`${button} mt-4`}>Submit a load <ArrowRight size={14} /></Link></div></div></Card> : query.loading ? <Loading error={query.error} retry={query.reload} /> : <div className="grid gap-5 lg:grid-cols-2">{query.data.map((trip) => <TripCard key={trip.id} trip={trip} role={role} />)}</div>}
       {open && <Modal title="Submit trip capacity" eyebrow="Carrier portal" onClose={() => setOpen(false)}><form onSubmit={submit} className="space-y-4"><div className="grid gap-4 sm:grid-cols-2"><LocationPicker label="Origin area or landmark" value={form.originLocation} countryCode={form.originCountry} onChange={setOrigin} onLabelChange={setOriginLabel} /><CountrySelect label="Origin country" value={form.originCountry} onChange={setOriginCountry} /><LocationPicker label="Destination area or landmark" value={form.destinationLocation} countryCode={form.destinationCountry} onChange={setDestination} onLabelChange={setDestinationLabel} /><CountrySelect label="Destination country" value={form.destinationCountry} onChange={setDestinationCountry} /><Field label="Departure date" type="date" value={form.departureDate} onChange={(value) => update("departureDate", value)} /><Field label="Departure time" type="time" value={form.departureTime} onChange={(value) => update("departureTime", value)} /><label><span className={labelClass}>Truck type</span><select className={input} value={form.vehicleType} onChange={(event) => update("vehicleType", event.target.value)}>{["Fuso", "Canter", "Trailer", "Flatbed"].map((item) => <option key={item}>{item}</option>)}</select></label><Field label="Available capacity (tons)" type="number" value={form.capacityTons} onChange={(value) => update("capacityTons", value)} /><Field label="Available space (m³)" type="number" value={form.capacityM3} onChange={(value) => update("capacityM3", value)} /><Field label="Trip price" type="number" value={form.price} onChange={(value) => update("price", value)} /><label><span className={labelClass}>Price currency</span><select className={input} value={form.currency} onChange={(event) => update("currency", event.target.value)}>{regionalReference.countries.map((country) => <option key={country.currency} value={country.currency}>{country.currency} · {country.name}</option>)}</select></label></div><p className="text-[11px] text-muted-foreground">Type any landmark, worksite, warehouse, or road area. The broker team will review the submission before matching.</p><button className={`${button} w-full`}><Plus size={14} /> Submit for broker review</button></form></Modal>}
    </div>
  );
}
...
function FreightPage() {
  ...
  return (
    <div className="space-y-6">
      <Header eyebrow="Shipper submission desk" title="My load submissions" detail="Submit the cargo, route, timing, and budget. Our broker team will qualify it and find the carrier." action={role === "Shipper" ? <button onClick={() => setOpen(true)} className={button}><Plus size={14} /> Submit a load</button> : undefined} />
      <ContextImage src="/branding/story/cargo-loading.jpg" alt="Cargo boxes being loaded beside a delivery truck" eyebrow="Broker-managed loads" title="You submit the shipment. The broker coordinates the move." />
      {role === "Carrier" ? <Card><div className="flex items-start gap-3"><ShieldCheck className="mt-0.5 text-accent-foreground" size={20} /><div><h3 className="font-display text-xl font-semibold">Loads are assigned by the broker desk</h3><p className="mt-1 text-sm leading-6 text-muted-foreground">The operations team will contact you when your submitted route and capacity are a fit. Loads are not exposed as an open marketplace.</p><Link href="/trips" className={`${button} mt-4`}>Submit trip capacity <ArrowRight size={14} /></Link></div></div></Card> : query.loading ? <Loading error={query.error} retry={query.reload} /> : <div className="grid gap-5 lg:grid-cols-2">{query.data.map((load) => <FreightCard key={load.id} load={load} role={role} />)}</div>}
       {open && <Modal title="Submit a load" eyebrow="Shipper portal" onClose={() => setOpen(false)}><form onSubmit={submit} className="space-y-4"><div className="grid gap-4 sm:grid-cols-2"><LocationPicker label="Pickup area or landmark" value={form.pickupLocation} countryCode={form.pickupCountry} onChange={setPickup} onLabelChange={setPickupLabel} /><CountrySelect label="Pickup country" value={form.pickupCountry} onChange={setPickupCountry} /><LocationPicker label="Drop-off area or landmark" value={form.dropoffLocation} countryCode={form.dropoffCountry} onChange={setDropoff} onLabelChange={setDropoffLabel} /><CountrySelect label="Drop-off country" value={form.dropoffCountry} onChange={setDropoffCountry} /><Field label="Cargo description" value={form.description} onChange={(value) => update("description", value)} placeholder="Bagged maize, cartons..." /><Field label="Cargo type" value={form.cargoType} onChange={(value) => update("cargoType", value)} /><Field label="Weight (tons)" type="number" value={form.weightTons} onChange={(value) => update("weightTons", value)} /><Field label="Volume (m³)" type="number" value={form.volumeM3} onChange={(value) => update("volumeM3", value)} /><Field label="Pickup date" type="date" value={form.pickupDate} onChange={(value) => update("pickupDate", value)} /><Field label="Budget" type="number" value={form.price} onChange={(value) => update("price", value)} /><label><span className={labelClass}>Budget currency</span><select className={input} value={form.currency} onChange={(event) => update("currency", event.target.value)}>{regionalReference.countries.map((country) => <option key={country.currency} value={country.currency}>{country.currency} · {country.name}</option>)}</select></label></div><p className="text-[11px] text-muted-foreground">Type any landmark, worksite, warehouse, or road area. The broker team will review the submission before matching.</p><button className={`${button} w-full`}><Plus size={14} /> Submit for broker review</button></form></Modal>}
    </div>
  );
}
...
function MatchesPage() {
  const query = useApi<CustomerRequest[]>("/requests", []);
  const [feedback, setFeedback] = useState("");
  const confirm = async (request: CustomerRequest) => {
    try {
      await api(`/requests/${request.id}/confirm`, { method: "POST", body: "{}" });
      setFeedback("Your confirmation was sent to the broker desk.");
      query.reload();
    } catch (reason: unknown) {
      setFeedback(reason instanceof Error ? reason.message : "The offer could not be confirmed.");
    }
  };
  const flow: BrokerRequest["status"][] = ["New", "Needs information", "Approved", "Matching", "Offer sent", "Confirmed", "Assigned", "In progress", "Closed"];
...
function AdminPage() {
  ...
  return <div className="admin-dashboard space-y-6">
    ...
    {operations.error && !adminPreview && <div className="rounded-lg border border-[#e4b4a9] bg-[#fbefeb] p-3 text-sm text-[#ad4339]">{operations.error}</div>}
      <div className="admin-primary-stats grid grid-cols-2 gap-3 md:grid-cols-5"><Stat label="Open requests" value={operations.data.metrics.totalOpen} note="Needs broker action" icon={Inbox} /><Stat label="New submissions" value={operations.data.metrics.newSubmissions} note="Not reviewed" icon={CircleAlert} accent /><Stat label="Needs attention" value={operations.data.metrics.needsAttention} note="Missing info or on hold" icon={Clock3} /><Stat label="Matching desk" value={operations.data.metrics.matching} note="Finding the right pair" icon={Gauge} /><Stat label="Active operations" value={operations.data.metrics.activeOperations} note="Assigned or moving" icon={RouteIcon} /></div>
      <div className="admin-finance-stats grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Stat label="Gross booking value" value={money(operations.data.metrics.grossVolume)} note="All broker-managed bookings" icon={Banknote} /><Stat label="Broker commission" value={money(operations.data.metrics.commissionValue)} note="12% earned on bookings" icon={BarChart3} /><Stat label="Unpaid bookings" value={operations.data.metrics.unpaidBookings} note="Payment follow-up" icon={LockKeyhole} accent={operations.data.metrics.unpaidBookings > 0} /><Stat label="Border issues" value={operations.data.metrics.borderIssues} note="At-border handoffs" icon={CircleAlert} accent={operations.data.metrics.borderIssues > 0} /></div>
     <div className="admin-workspace grid gap-5 xl:grid-cols-[.92fr_1.35fr]">
       <Card className="admin-queue-card p-0">
         <div className="admin-queue-header border-b border-border p-4"><div className="flex items-center justify-between"><div><p className={labelClass}>Broker queue</p><h2 className="font-display text-xl font-semibold">Requests to work</h2></div><Filter size={17} className="text-muted-foreground" /></div><div className="admin-queue-filters mt-4 flex flex-wrap gap-2">{["All", "Attention", "Matching desk", "In progress", "Closed"].map((item) => <button type="button" key={item} onClick={() => setFilter(item)} className={`admin-queue-filter rounded-full border px-2.5 py-1.5 text-[10px] font-bold ${filter === item ? "border-primary bg-primary text-primary-foreground" : "border-border text-muted-foreground hover:bg-muted"}`}>{item}</button>)}</div></div>
         <div className="admin-queue-list max-h-[720px] overflow-y-auto p-3">{operations.loading && !operations.data.requests.length ? <Loading error={operations.error} retry={operations.reload} /> : visibleRequests.length ? <div className="space-y-2">{visibleRequests.map((request) => <button type="button" key={request.id} onClick={() => setSelectedId(request.id)} className={`admin-request-card w-full rounded-xl border p-3 text-left transition ${selectedId === request.id ? "border-primary bg-[#e5eee9] shadow-sm" : "border-border hover:border-primary/40 hover:bg-muted/40"}`}><div className="admin-request-card-head flex items-start justify-between gap-3"><div className="admin-request-card-copy min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="font-mono-ui text-[9px] font-bold uppercase tracking-[.14em] text-muted-foreground">{request.kind} · {request.priority}</span>{request.priority === "Urgent" && <span className="rounded-full bg-[#fbefeb] px-2 py-0.5 text-[9px] font-bold text-[#ad4339]">Urgent</span>}</div><p className="admin-request-title mt-2 truncate text-sm font-bold">{request.title}</p><p className="admin-request-route mt-1 truncate text-[11px] text-muted-foreground">{request.counterpart} · {request.corridor}</p></div><span className="admin-request-status"><Status value={request.status} /></span></div><div className="admin-request-footer mt-3 flex items-center justify-between border-t border-border/70 pt-2 text-[10px] text-muted-foreground"><span>{dateFmt(request.date)}</span><span>{request.suggestions.length} suggested match{request.suggestions.length === 1 ? "" : "es"}</span></div></button>)}</div> : <div className="p-6 text-center text-sm text-muted-foreground">No requests in this queue.</div>}</div>
      </Card>
      <div className="space-y-5">
         {selected ? <Card className="admin-detail-card">
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2"><label><span className={labelClass}>Workflow status</span><select className={input} value={selected.status} onChange={(event) => void updateRequest({ status: event.target.value as BrokerRequest["status"] })}>{["New", "Needs information", "Approved", "Matching", "Offer sent", "Confirmed", "Assigned", "In progress", "On hold", "Closed", "Rejected"].map((item) => <option key={item}>{item}</option>)}</select></label><label><span className={labelClass}>Priority</span><select className={input} value={selected.priority} onChange={(event) => void updateRequest({ priority: event.target.value as BrokerRequest["priority"] })}>{["Low", "Normal", "High", "Urgent"].map((item) => <option key={item}>{item}</option>)}</select></label></div>
            <div className="admin-detail-actions mt-4 flex flex-wrap gap-2"><button type="button" onClick={() => void updateRequest({ status: "Approved" })} className={secondaryButton}><Check size={14} /> Approve intake</button><button type="button" onClick={() => void updateRequest({ status: "Matching" })} className={button}><Gauge size={14} /> Move to matching</button><button type="button" onClick={() => void updateRequest({ status: "Needs information" })} className={secondaryButton}><MessageSquare size={14} /> Request information</button>{selected.proposedMatchId && !selected.bookingId && <button type="button" onClick={() => void assignBooking()} className={button}><ClipboardCheck size={14} /> Create booking</button>}{selected.bookingId && <span className="inline-flex items-center gap-1.5 rounded-lg bg-[#e5f1e9] px-3 py-2 text-xs font-bold text-[#28765a]"><Check size={14} /> Booking {selected.bookingId}</span>}</div>
           <div className="mt-6"><p className={labelClass}>Activity timeline</p><div className="mt-3 space-y-3">{selected.activities?.length ? selected.activities.map((activity) => <div key={activity.id} className="flex gap-3 border-l-2 border-accent/40 pl-3"><span className="mt-1.5 h-2 w-2 shrink-0 rounded-full bg-accent" /><div><p className="text-xs font-bold">{activity.label}</p><p className="mt-0.5 text-[11px] leading-5 text-muted-foreground">{activity.detail}</p><p className="mt-1 font-mono-ui text-[9px] uppercase tracking-wider text-muted-foreground">{activity.actor} · {dateFmt(activity.createdAt)}</p></div></div>) : <p className="text-sm text-muted-foreground">No activity recorded yet.</p>}</div></div>
           <div className="mt-6"><p className={labelClass}>Admin notes</p><div className="space-y-2">{selected.notes.map((item, index) => <div key={`${item}-${index}`} className="rounded-lg bg-muted/60 p-3 text-xs leading-5">{item}</div>)}</div><div className="admin-note-composer mt-3 flex gap-2"><input className={input} value={note} onChange={(event) => setNote(event.target.value)} placeholder="Add an internal note..." /><button type="button" disabled={!note.trim()} onClick={() => void updateRequest({ note: note.trim() })} className={button}><Plus size={14} /> Add</button></div></div>
         ...
         {selected && <Card className="admin-matches-card"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className={labelClass}>Suggested matches</p><h3 className="font-display text-xl font-semibold">Admin chooses the pairing</h3><p className="mt-1 text-xs text-muted-foreground">Suggestions are ranked by corridor, date, capacity, and fit. Nothing is booked automatically.</p></div><Search size={18} className="shrink-0 text-accent-foreground" /></div><div className="mt-4 space-y-3">{selected.suggestions.length ? selected.suggestions.map((suggestion) => <div key={suggestion.id} className="admin-suggestion flex flex-col gap-3 rounded-xl border border-border p-3 sm:flex-row sm:items-center sm:justify-between"><div className="admin-suggestion-thumb"><img src={suggestion.kind === "Trip" ? truckAsset(suggestion.title.split(" ")[0]) : "/branding/story/cargo-loading.jpg"} alt="" /></div><div className="min-w-0"><div className="flex items-center gap-2"><span className="rounded-full bg-[#e5f1e9] px-2 py-1 font-mono-ui text-[9px] font-bold text-[#28765a]">{suggestion.compatibility}% fit</span><span className="font-mono-ui text-[9px] uppercase tracking-wider text-muted-foreground">{suggestion.kind}</span></div><p className="mt-2 text-sm font-bold">{suggestion.title}</p><p className="mt-1 text-[11px] text-muted-foreground">{suggestion.counterpart} · {suggestion.corridor} · {dateFmt(suggestion.date)}</p></div><div className="admin-suggestion-action flex items-center justify-between gap-3 sm:justify-end"><strong className="font-display text-lg">{money(suggestion.price)}</strong><button type="button" onClick={() => void sendOffer(suggestion)} className={button}>Send offer <ArrowRight size={14} /></button></div></div>) : <div className="rounded-lg bg-muted/50 p-4 text-sm text-muted-foreground">No suitable candidate yet. Keep this request in the matching queue.</div>}</div></Card>}
         ...
         <Card><div className="flex items-center justify-between"><div><p className={labelClass}>Compliance queue</p><h3 className="font-display text-xl font-semibold">Verification review</h3></div><ShieldCheck size={17} className="text-accent-foreground" /></div><div className="mt-4 space-y-3">{operations.data.verifications.length ? operations.data.verifications.slice(0, 4).map((item) => <div key={item.id} className="flex items-center justify-between gap-3 border-t border-border pt-3"><div className="min-w-0"><p className="truncate text-xs font-bold">{item.name}</p><p className="mt-1 truncate text-[11px] text-muted-foreground">{item.licenseNumber} · submitted {dateFmt(item.submittedAt)}</p></div><Status value={item.status} /></div>) : <p className="text-sm text-muted-foreground">No verification submissions.</p>}</div></Card>
      </div>
    </div>
  </div>;
}
