...
const nav = [
  ...
  ["/matches", "Smart matching", Gauge], ["/bookings", "Bookings & payout", LockKeyhole], ["/tracking", "Live tracker", MapPin],
  ["/messages", "Messages", MessageSquare], ["/documents", "Document hub", FileCheck2], ["/verification", "Driver verification", ShieldCheck],
  ["/admin", "Admin control", UsersRound], ["/payments", "Mobile money", Banknote], ["/regional", "EAC network", Globe2],
] as const;
...
function Shell({ children }: { children: ReactNode }) {
  ...
  return <RoleContext.Provider value={role}><div className="noise min-h-[100dvh] bg-background">
    <aside className={`fixed inset-y-0 left-0 z-40 flex w-[258px] flex-col bg-sidebar px-4 py-5 text-sidebar-foreground shadow-2xl transition-transform lg:translate-x-0 ${mobileOpen ? "translate-x-0" : "-translate-x-full"}`}>
      ...
      <div className="mt-auto space-y-3"><div className="rounded-xl border border-sidebar-border bg-sidebar-accent/50 p-3"><div className="flex items-center gap-2 text-[11px] font-semibold"><span className="live-dot h-2 w-2 rounded-full bg-[#65c7a1]" /> EAC border network live</div><p className="mt-2 text-[11px] leading-relaxed text-sidebar-foreground/45">4 regional corridors syncing · settlement ready</p></div><div className="flex items-center gap-3 border-t border-sidebar-border px-2 pt-4"><div className="flex h-8 w-8 items-center justify-center rounded-full bg-[#d9a35c] text-xs font-bold text-primary">NS</div><div className="min-w-0 flex-1"><p className="truncate text-xs font-semibold">Nadia S.</p><p className="truncate text-[10px] text-sidebar-foreground/45">{role} workspace</p></div></div></div>
    </aside>
     <MobileHeader notificationsOpen={notificationsOpen} onNotifications={() => setNotificationsOpen((open) => !open)} onAuth={() => setAuthOpen(true)} />
...
function EacNetworkBanner() {
  const query = useApi<EacReference>("/reference/eac", regionalReference);
  const reference = query.data;
  return <section className="mb-5 rounded-xl border border-[#d9c29c] bg-[#fff8e9] p-4 text-primary shadow-sm sm:p-5"><div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div className="flex items-start gap-3"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-[#f0d49a] text-[#7d541c]"><Globe2 size={19} /></span><div><p className="font-mono-ui text-[10px] uppercase tracking-[.16em] text-[#9a641c]">East Africa Community network</p><h3 className="mt-1 font-display text-lg font-semibold">Cross-border freight is now the operating model</h3><p className="mt-1 text-xs leading-relaxed text-primary/65">{reference.countries.length} supported countries · {reference.corridors.length} starter corridors · local settlement currencies and border milestones.</p></div></div><div className="flex flex-wrap gap-2">{reference.corridors.map((corridor) => <span key={`${corridor.originCountry}-${corridor.destinationCountry}`} className="rounded-full border border-[#e4c994] bg-white/70 px-2.5 py-1 font-mono-ui text-[9px] font-bold text-[#7d541c]">{corridor.origin} → {corridor.destination}</span>)}</div></div></section>;
}
...
function TripsPage() {
  ...
  const [form, setForm] = useState({ origin: "Kampala", destination: "Mbale", departureDate: "2026-09-10", departureTime: "07:00", vehicleType: "Fuso", capacityTons: "8", capacityM3: "42", price: "680000", priceType: "Fixed" });
  ...
  return <div className="space-y-6"><Header eyebrow="Carrier portal" title="Return trips" detail="Turn the empty leg home into paid capacity across Kampala, Mbale, Mbarara, Gulu, and Malaba." action={<button onClick={() => setOpen(true)} className={button}><Plus size={14} /> Post return trip</button>} />{query.loading ? <Loading error={query.error} retry={query.reload} /> : <div className="grid gap-4 lg:grid-cols-2">{query.data.map((trip) => <Card key={trip.id}><div className="flex items-start justify-between gap-3"><div><p className="font-mono-ui text-[10px] uppercase tracking-wider text-muted-foreground">Departure · {dateFmt(trip.departureDate)} {trip.departureTime}</p><h3 className="mt-2 font-display text-xl font-semibold">{trip.origin} <ArrowRight className="mx-1 inline text-accent-foreground" size={16} /> {trip.destination}</h3><p className="mt-1 text-xs text-muted-foreground">{trip.carrier} · {trip.vehicleType} · {trip.carrierRating} ★</p></div><Status value={trip.status} /></div><div className="mt-5 grid grid-cols-3 gap-3 border-t border-border pt-4"><div><p className={labelClass}>Available</p><p className="font-display text-lg font-semibold">{trip.capacityTons} t</p></div><div><p className={labelClass}>Space</p><p className="font-display text-lg font-semibold">{trip.capacityM3} m³</p></div><div><p className={labelClass}>Rate</p><p className="font-display text-lg font-semibold">{money(trip.price)}</p></div></div><div className="mt-4 flex gap-2"><Link href="/matches" className={secondaryButton}>Find matching loads <ChevronRight size={14} /></Link><a href="tel:+256700000000" className={secondaryButton}><Phone size={14} /> Call</a></div></Card>)}</div>}{open && <Modal title="Post a return trip" eyebrow="Carrier portal" onClose={() => setOpen(false)}><form onSubmit={submit} className="space-y-4"><div className="grid gap-4 sm:grid-cols-2"><label><span className={labelClass}>Origin</span><select className={input} value={form.origin} onChange={(event) => update("origin", event.target.value)}>{["Kampala", "Mbale", "Mbarara", "Gulu", "Malaba"].map((city) => <option key={city}>{city}</option>)}</select></label><label><span className={labelClass}>Destination</span><select className={input} value={form.destination} onChange={(event) => update("destination", event.target.value)}>{["Mbale", "Mbarara", "Gulu", "Kampala", "Malaba"].map((city) => <option key={city}>{city}</option>)}</select></label><Field label="Departure date" type="date" value={form.departureDate} onChange={(value) => update("departureDate", value)} /><Field label="Departure time" type="time" value={form.departureTime} onChange={(value) => update("departureTime", value)} /><label><span className={labelClass}>Truck type</span><select className={input} value={form.vehicleType} onChange={(event) => update("vehicleType", event.target.value)}>{["Fuso", "Canter", "Trailer", "Flatbed"].map((item) => <option key={item}>{item}</option>)}</select></label><Field label="Available capacity (tons)" type="number" value={form.capacityTons} onChange={(value) => update("capacityTons", value)} /><Field label="Available space (m³)" type="number" value={form.capacityM3} onChange={(value) => update("capacityM3", value)} /><Field label="Trip price (UGX)" type="number" value={form.price} onChange={(value) => update("price", value)} /></div><button className={`${button} w-full`}><Plus size={14} /> Publish trip</button></form></Modal>}</div>;
}

function FreightPage() {
  const query = useApi<Freight[]>("/freight", []);
  const [open, setOpen] = useState(false);
  const [form, setForm] = useState({ pickup: "Kampala", dropoff: "Mbale", description: "", cargoType: "General cargo", weightTons: "2", volumeM3: "8", dimensions: "2 pallets", pickupDate: "2026-09-10", price: "400000" });
  const update = (key: string, value: string) => setForm((current) => ({ ...current, [key]: value }));
  ...
  return <div className="space-y-6"><Header eyebrow="Shipper portal" title="Load board" detail="Post cargo once and let verified carriers find the right route, date, and capacity." action={<button onClick={() => setOpen(true)} className={button}><Plus size={14} /> Post a load</button>} />{query.loading ? <Loading error={query.error} retry={query.reload} /> : <div className="grid gap-4 lg:grid-cols-2">{query.data.map((load) => <Card key={load.id}><div className="flex items-start justify-between gap-3"><div><p className="font-mono-ui text-[10px] uppercase tracking-wider text-muted-foreground">{load.cargoType || "General cargo"} · Pickup {dateFmt(load.pickupDate)}</p><h3 className="mt-2 font-display text-xl font-semibold">{load.pickup} <ArrowRight className="mx-1 inline text-accent-foreground" size={16} /> {load.dropoff}</h3><p className="mt-1 text-xs text-muted-foreground">{load.shipper} · {load.description}</p></div><Status value={load.status} /></div><div className="mt-5 grid grid-cols-3 gap-3 border-t border-border pt-4"><div><p className={labelClass}>Weight</p><p className="font-display text-lg font-semibold">{load.weightTons} t</p></div><div><p className={labelClass}>Volume</p><p className="font-display text-lg font-semibold">{load.volumeM3 || "—"} m³</p></div><div><p className={labelClass}>Budget</p><p className="font-display text-lg font-semibold">{money(load.price)}</p></div></div><Link href="/matches" className={`${secondaryButton} mt-4`}>Search matching trucks <Search size={14} /></Link></Card>)}</div>}{open && <Modal title="Post a load" eyebrow="Shipper portal" onClose={() => setOpen(false)}><form onSubmit={submit} className="space-y-4"><div className="grid gap-4 sm:grid-cols-2"><label><span className={labelClass}>Pickup</span><select className={input} value={form.pickup} onChange={(event) => update("pickup", event.target.value)}>{["Kampala", "Mbale", "Mbarara", "Gulu", "Malaba"].map((city) => <option key={city}>{city}</option>)}</select></label><label><span className={labelClass}>Destination</span><select className={input} value={form.dropoff} onChange={(event) => update("dropoff", event.target.value)}>{["Mbale", "Mbarara", "Gulu", "Kampala", "Malaba"].map((city) => <option key={city}>{city}</option>)}</select></label><Field label="Cargo description" value={form.description} onChange={(value) => update("description", value)} placeholder="Bagged maize, cartons..." /><Field label="Cargo type" value={form.cargoType} onChange={(value) => update("cargoType", value)} /><Field label="Weight (tons)" type="number" value={form.weightTons} onChange={(value) => update("weightTons", value)} /><Field label="Volume (m³)" type="number" value={form.volumeM3} onChange={(value) => update("volumeM3", value)} /><Field label="Pickup date" type="date" value={form.pickupDate} onChange={(value) => update("pickupDate", value)} /><Field label="Budget (UGX)" type="number" value={form.price} onChange={(value) => update("price", value)} /></div><button className={`${button} w-full`}><Plus size={14} /> Publish load</button></form></Modal>}</div>;
}
...
function TrackingPage() {
  ...
  return <div className="space-y-6"><Header eyebrow="Live operations" title="Delivery tracker" detail="Follow pickup, transit, border crossing, and destination handoff from one route view." action={<a href="tel:+256700000000" className={secondaryButton}><Phone size={14} /> Call driver</a>} /><div className="grid gap-5 lg:grid-cols-[1.3fr_.7fr]"><Card><div className="paper-grid relative h-[360px] overflow-hidden rounded-lg border border-border bg-[#f4f0e7]"><svg className="absolute inset-0 h-full w-full" viewBox="0 0 800 360" preserveAspectRatio="none"><path d="M92 256 C220 215, 235 72, 382 115 S535 260, 710 116" fill="none" stroke="#d7984e" strokeDasharray="6 8" strokeWidth="4" /><path d="M92 256 C220 215, 235 72, 382 115 S535 260, 710 116" fill="none" stroke="#2d8066" strokeDasharray="2 12" strokeWidth="2" /></svg>{["Pickup", "Transit", "Border", "Destination"].map((item, index) => <div key={item} className={`absolute ${["left-[10%] bottom-[25%]", "left-[29%] top-[17%]", "left-[48%] top-[28%]", "right-[9%] top-[27%]"][index]} text-center`}><span className={`mx-auto flex h-8 w-8 items-center justify-center rounded-full ${index < 2 ? "bg-accent text-primary" : "bg-card text-primary"} ring-4 ring-background`}><MapPin size={15} /></span><p className="mt-2 font-mono-ui text-[9px] font-bold uppercase">{item}</p></div>)}<div className="absolute left-5 top-5 rounded-lg border border-border bg-card/90 p-3"><p className="font-mono-ui text-[9px] text-muted-foreground">ACTIVE ROUTE</p><p className="mt-1 font-display text-lg font-semibold">{active?.corridor || "Kampala → Mbale"}</p><p className="mt-1 text-[11px] text-muted-foreground">{active?.status || "Awaiting booking"}</p></div></div></Card><Card><p className={labelClass}>Handoff timeline</p><h3 className="mt-1 font-display text-xl font-semibold">Proof at every step</h3><div className="mt-6 space-y-5">{["En Route to Pickup", "Goods Loaded", "In Transit", "Arrived for Unloading"].map((step, index) => <div key={step} className="flex gap-3"><span className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full ${index < 2 ? "bg-[#e4f1ea] text-[#28765a]" : "bg-muted text-muted-foreground"}`}>{index < 2 ? <Check size={14} /> : index + 1}</span><div><p className="text-xs font-bold">{step}</p><p className="mt-1 text-[11px] text-muted-foreground">{index < 2 ? "Recorded" : "Waiting for driver update"}</p></div></div>)}</div><Link href="/bookings" className={`${button} mt-7 w-full`}>Manage handoff <ArrowRight size={14} /></Link></Card></div></div>;
}
