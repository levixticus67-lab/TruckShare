  ...
  return <RoleContext.Provider value={role}><div className="noise min-h-[100dvh] bg-background">
    <aside className={`fixed inset-y-0 left-0 z-40 flex w-[258px] flex-col bg-sidebar px-4 py-5 text-sidebar-foreground shadow-2xl transition-transform lg:translate-x-0 ${mobileOpen ? "translate-x-0" : "-translate-x-full"}`}>
      ...
      <nav className={`mobile-bottom-nav fixed inset-x-3 bottom-3 z-40 flex h-[70px] items-center gap-1 rounded-[22px] border border-border/80 px-2 shadow-xl lg:hidden ${mobileOpen ? "pointer-events-none opacity-0" : ""}`} aria-label="Mobile navigation">
         <Link href="/" onClick={() => setMobileOpen(false)} aria-current={location === "/" ? "page" : undefined} className={`mobile-nav-item ${location === "/" ? "is-active" : ""}`}><LayoutDashboard size={18} /><span>Home</span>{location === "/" && <i aria-hidden="true" />}</Link>
...
function ContextImage({ src, alt, eyebrow, title }: { src: string; alt: string; eyebrow: string; title: string }) {
  return <div className="relative min-h-[170px] overflow-hidden rounded-xl border border-border shadow-sm sm:min-h-[205px]">
...
function TripsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <ContextImage src="/branding/story/carrier-road.jpg" alt="Truck travelling along a winding road" eyebrow={role === "Shipper" ? "Available capacity" : "Carrier network"} title={role === "Shipper" ? "Find the truck that fits your route." : "Turn every route into useful capacity."} />
...
function FreightPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <ContextImage src="/branding/story/cargo-loading.jpg" alt="Cargo boxes being loaded beside a delivery truck" eyebrow={role === "Carrier" ? "Cargo to move" : "Shipper workspace"} title={role === "Carrier" ? "Find cargo that fits the space you already have." : "Give every shipment a clear route forward."} />
